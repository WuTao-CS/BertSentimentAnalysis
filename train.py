...
from transformers import AutoConfig, AutoTokenizer,BertTokenizer,BertConfig
from modeling import BertCNNForSentimentClassification, BertForSentimentClassification, AlbertForSentimentClassification, BertLSTMForSentimentClassification, DistilBertForSentimentClassification
...
if __name__ == "__main__":
	...
	if args.model_name_or_path is None:
		args.model_name_or_path = "./Pre_Train_Model"

	#读取预训练模型的config文件
	config = BertConfig.from_pretrained(args.model_name_or_path+'/config.json')
	...
	if config.model_type == 'bert':
		...
		model = BertCNNForSentimentClassification.from_pretrained(args.model_name_or_path, config=config)
	elif config.model_type == 'bert_lstm':
		model = BertLSTMForSentimentClassification.from_pretrained(args.model_name_or_path, config=config)
	...
	train_set = SSTDataset(filename='data/all/train.tsv', maxlen=args.maxlen_train, tokenizer=tokenizer)
	val_set = SSTDataset(filename='data/all/dev.tsv', maxlen=args.maxlen_val, tokenizer=tokenizer)
